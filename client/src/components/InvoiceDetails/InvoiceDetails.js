...
const InvoiceDetails = () => {
  ...
  const createAndDownloadPdf = () => {
    ...
    // Prepare the data for the PDF (you can adjust this part as per your invoice data)
    const invoiceData = {
      ...
      notes: invoice.notes,
      subTotal: (Math.round(invoice.subTotal * 10) / 10).toLocaleString(),
      total: (Math.round(invoice.total * 10) / 10).toLocaleString(),
      type: invoice.type,
      vat: invoice.vat,
      items: invoice.items,
    ...
    // Set PDF options for width, height, and margins
    const options = {
      margin: [7, 0, 3, 0], // top, left, bottom, right margin
      filename: "invoice.pdf",
      image: { type: "jpeg", quality: 0.98 }, // High-quality image output
      html2canvas: {
        dpi: 300, // Increase DPI for higher quality rendering (default: 96)
        letterRendering: true, // Render letters more clearly
      ...
      jsPDF: {
        ...
        compressPdf: true, // Option to compress the PDF for a smaller file size while keeping quality
      },
    };
    ...
    html2pdf()
      .from(element)
      .set(options) // Apply the PDF options
      .save("invoice.pdf")
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center">
              <img
                ...
                alt="Company Logo"
                className="h-[90px] w-[100px] rounded-lg object-cover ring-2 ring-green-200"
              />
              <div className="ml-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 px-8 py-6 border-b border-purple-200">
          <div className="bg-gradient-to-br from-purple-50 to-pink-50 p-4 rounded-lg border border-purple-100">
            ...
            <p className="text-purple-700">
             Name : {invoice?.businessDetails?.data?.data?.businessName}
              <br />
              Gmail : {invoice?.businessDetails?.data?.data?.email}
              <br />
              Phone : {invoice?.businessDetails?.data?.data?.phoneNumber}
              <br />
             Address :   {invoice?.businessDetails?.data?.data?.address}
            </p>
          </div>
          <div className="bg-gradient-to-br from-purple-50 to-pink-50 p-4 rounded-lg border border-purple-100">
