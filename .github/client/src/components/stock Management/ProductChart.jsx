...
useEffect(() => {
  if (products.length > 0) {
    const dynamicColumns = Object.keys(products[0] || {}).map((key) => ({
      ...
      valueGetter: (params) => {
        if (key === "category") {
          return params.data?.category?.categoryName || "No Category Added";
        } else if (key === "CGST") {
          return params.data?.category?.CGST || "-";
        } else if (key === "SGST") {
          return params.data?.category?.SGST || "-";
        }
    ...
    // ✅ Add CGST and SGST separately (if they are missing in products[0])
    if (!products[0]?.CGST) {
      dynamicColumns.push({
        headerName: "CGST",
        field: "CGST",
        valueGetter: (params) => `${params.data?.category?.CGST} %` || " - ",
      });
    }
    if (!products[0]?.SGST) {
      dynamicColumns.push({
        headerName: "SGST",
        field: "SGST",
        valueGetter: (params) => `${params.data?.category?.SGST} %` || " - ",
      });
    }
